...
@section('breadcrumb')
<div class="col-sm-6">
    <h4 class="page-title">Market Oppo</h4>
</div>
...
<script type="text/javascript">
    ...
    function detailOppo(id) {
        $.get('{{asset('project/edit')}}/'+id,function (data) {
            ...
            $('#package').val(data[0].Oppo_package);
            $('#buildinfo_store_name_x').val(data[2].store_name);
            $('#ads_id').val(Oppo_ads.ads_id);
            $('#banner').val(Oppo_ads.ads_banner);
            $('#ads_inter').val(Oppo_ads.ads_inter);
            $('#ads_reward').val(Oppo_ads.ads_reward);
            $('#ads_native').val(Oppo_ads.ads_native);
            $('#ads_open').val(Oppo_ads.ads_open);
